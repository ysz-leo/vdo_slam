...
bool readFLOFile(const std::string& filename, cv::Mat& imFlow) {
    ...
    // 检查文件标识符是否为202021.25
    if (std::abs(flag - 202021.25f) > std::numeric_limits<float>::epsilon()) {
        std::cerr << "Invalid FLO file signature." << std::endl;
        return false;
    }
    ...
    // 检查宽度和高度是否有效
    if (width <= 0 || height <= 0) {
        std::cerr << "Invalid FLO file dimensions." << std::endl;
        return false;
    }

    // 创建用于存储光流数据的 Mat 对象
    imFlow.create(height, width, CV_32FC2);
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            ...
            imFlow.at<cv::Vec2f>(i, j) = cv::Vec2f(u, v);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        cv::Mat imFlow; // 声明 Mat 对象
        if (readFLOFile(vstrFilenamesFLO[ni], imFlow)) {
            // imFlow 现在包含了从 FLO 文件中读取的光流数据
        } else {
            // 处理错误
        }
        // FlowShow(imFlow);
