                                ...
                                const double &timestamp, cv::Mat &imTraj, const int &nImage)
{
    ...
    if(timestamp!=0 && bFrame2Frame == true)
    {
        ...
        for (int i = 0; i < TemperalMatch_subset.size(); i=i+1)
        {
            ...
            if(maskSEM.at<int>(KeyPoints_tmp[0].pt.y,KeyPoints_tmp[0].pt.x)!=0)
                continue;
            cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(0,0,0), (DrawMatchesFlags)1); // red
        }
        ...
        for (int i = 0; i < mCurrentFrame.vObjLabel.size(); ++i)
        {
            ...
            {
                case 0:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(0,0,255), (DrawMatchesFlags)1); // red
                    break;
                case 1:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(128, 0, 128), (DrawMatchesFlags)1); // 255, 165, 0
                    break;
                case 2:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(255,255,0), (DrawMatchesFlags)1);
                    break;
                case 3:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(0, 255, 0), (DrawMatchesFlags)1); // 255,255,0
                    break;
                case 4:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(255,0,0), (DrawMatchesFlags)1); // 255,192,203
                    break;
                case 5:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(0,255,255), (DrawMatchesFlags)1);
                    break;
                case 6:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(128, 0, 128), (DrawMatchesFlags)1);
                    break;
                case 7:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(255,255,255), (DrawMatchesFlags)1);
                    break;
                case 8:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(255,228,196), (DrawMatchesFlags)1);
                    break;
                case 9:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(180, 105, 255), (DrawMatchesFlags)1);
                    break;
                case 10:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(165,42,42), (DrawMatchesFlags)1);
                    break;
                case 11:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(35, 142, 107), (DrawMatchesFlags)1);
                    break;
                case 12:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(45, 82, 160), (DrawMatchesFlags)1);
                    break;
                case 13:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(0,0,255), (DrawMatchesFlags)1); // red
                    break;
                case 14:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(255, 165, 0), (DrawMatchesFlags)1);
                    break;
                case 15:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(0,255,0), (DrawMatchesFlags)1);
                    break;
                case 16:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(255,255,0), (DrawMatchesFlags)1);
                    break;
                case 17:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(255,192,203), (DrawMatchesFlags)1);
                    break;
                case 18:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(0,255,255), (DrawMatchesFlags)1);
                    break;
                ...
                case 25:
                    ...
                    break;
                case 41:
                    cv::drawKeypoints(imRGB, KeyPoints_tmp, imRGB, cv::Scalar(60, 20, 220), (DrawMatchesFlags)1);
                    break;
            }
        }
    ...
    if(timestamp!=0 && bFrame2Frame == true && mTestData==KITTI)
    {
        ...
        cvtColor(mImGray, mImBGR, COLOR_GRAY2RGB);
    ...
    if (mTestData==KITTI)
    {
        ...
        cv::rectangle(imTraj, cv::Point(10, 30), cv::Point(550, 60), CV_RGB(0,0,0), FILLED);
